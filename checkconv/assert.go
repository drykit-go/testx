...
// FromBool returns a check.Checker[interface{}] that wraps the given
// check.Checker[bool], so it can be used as a generic checker.
func FromBool(c check.Checker[bool]) check.Checker[interface{}] {
...
// FromBytes returns a check.Checker[interface{}] that wraps the given
// check.Checker[[]byte], so it can be used as a generic checker.
func FromBytes(c check.Checker[[]byte]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromString returns a check.Checker[interface{}] that wraps the given
// check.Checker[string], so it can be used as a generic checker.
func FromString(c check.Checker[string]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromInt returns a check.Checker[interface{}] that wraps the given
// check.Checker[int], so it can be used as a generic checker.
func FromInt(c check.Checker[int]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromFloat64 returns a check.Checker[interface{}] that wraps the given
// check.Checker[float64], so it can be used as a generic checker.
func FromFloat64(c check.Checker[float64]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromDuration returns a check.Checker[interface{}] that wraps the given
// check.Checker[time.Duration], so it can be used as a generic checker.
func FromDuration(c check.Checker[time.Duration]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromContext returns a check.Checker[interface{}] that wraps the given
// check.Checker[context.Context], so it can be used as a generic checker.
func FromContext(c check.Checker[context.Context]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromHTTPHeader returns a check.Checker[interface{}] that wraps the given
// check.Checker[http.Header], so it can be used as a generic checker.
func FromHTTPHeader(c check.Checker[http.Header]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromHTTPRequest returns a check.Checker[interface{}] that wraps the given
// check.Checker[*http.Request], so it can be used as a generic checker.
func FromHTTPRequest(c check.Checker[*http.Request]) check.Checker[interface{}] {
	return check.NewChecker(
...
// FromHTTPResponse returns a check.Checker[interface{}] that wraps the given
// check.Checker[*http.Response], so it can be used as a generic checker.
func FromHTTPResponse(c check.Checker[*http.Response]) check.Checker[interface{}] {
	return check.NewChecker(
